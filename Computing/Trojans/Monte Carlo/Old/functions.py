...
def counter_rotate_monte_carlo(asteroids, ticks, a):
    hit = []
    escape = []
    ...
    for i in range(0,len(asteroids)):
        ...
        for t in range(0, ticks):
            ...
            if x_0>0 and y_0<0:
                theta = 2*np.pi+np.arctan(y_0/x_0)-theta_t
            radius = modVector(asteroid[t])
            x_1 = 1*radius*np.cos(theta)
            y_1 = 1*radius*np.sin(theta)
            if radius>1.15*r_J or radius<0.85*r_J:
                print('radius for',+i)
                escape.append(i)
                break
            if x_1<0 and y_1>0 and t<ticks/6:
                escape.append(i)
                break
            if x_1<0 and y_1>0 and t>ticks/2:
                hit.append(i)
                break
