...
heatmap = plt.pcolormesh(x, y, Z, vmin=-265000000, vmax=-255900000, cmap='binary')
plt.contour(x,y,Z, levels=contourlevelmanual, cmap = 'gray')
...
########################################
ax4 = plt.subplot(8,8,(6,48))
total=np.load('total.npy')
hit=np.load('hit.npy')
escape=np.load('escape.npy')

for i in range(0,len(total)):
    ax4.scatter(total[i][0],total[i][1],color='blue',s=0.9)
for i in range(0,len(hit)):
    ax4.scatter(hit[i][0],hit[i][1],color='lime',s=2.5,zorder=1)
for i in range(0,len(escape)):
    ax4.scatter(escape[i][0],escape[i][1],color='black',s=1.1)
