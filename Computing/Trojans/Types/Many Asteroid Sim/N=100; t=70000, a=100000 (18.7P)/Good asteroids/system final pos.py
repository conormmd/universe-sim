...
pbar=ProgressBar()
plt.scatter(planets[0][-1,0], planets[0][-1,1], label='Sun')
plt.scatter(planets[1][-1,0], planets[1][-1,1], color='black',label='Jupiter')
for i in pbar(good_asteroids):
    plt.scatter(asteroids[i][-1,0], asteroids[i][-1,1])
